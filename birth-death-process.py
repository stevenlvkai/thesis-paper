...
if __name__=="__main__":
    ...
    coefficient=np.arange(7,107,1)
    time1=np.arange(0,1000,1)
    time1, coefficient = np.meshgrid(time1, coefficient)
    Z=data.values
    Z=np.transpose(Z)
    surf = ax.plot_surface(time1, coefficient, Z, cmap=cm.coolwarm,
    ...
    ax.set_xlabel('Date')
    ax.set_ylabel('Death Interval')
    ax.set_zlabel('Remaining Cryptocurrency')
    ax.view_init(elev=20., azim=-50)
    fig.colorbar(surf, shrink=1, aspect=10)
    fig.set_size_inches(18.5,10.5)    
    fig.savefig('D:\吕凯\中央财经大学\毕业论文\生灭过程\power.png', dpi=250)
    
    plt.plot(range(7,107),average)
